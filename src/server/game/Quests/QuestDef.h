...
#ifndef TRILLIUMEMU_QUEST_H
#define TRILLIUMEMU_QUEST_H
...
enum __QuestFlags
{
    ...
    QUEST_TRILLIUMEMU_FLAGS_REPEATABLE           = 0x00100000,   // Set by 1 in SpecialFlags from DB
    QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT = 0x00200000,   // Set by 2 in SpecialFlags from DB (if reequired area explore, spell SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command SCRIPT_COMMAND_QUEST_EXPLORED use, set from script)
    QUEST_TRILLIUMEMU_FLAGS_AUTO_ACCEPT          = 0x00400000,  // Set by 4 in SpecialFlags in DB if the quest is to be auto-accepted.
    QUEST_TRILLIUMEMU_FLAGS_DF_QUEST             = 0x00800000,  // Set by 8 in SpecialFlags in DB if the quest is used by Dungeon Finder.

    QUEST_TRILLIUMEMU_FLAGS_DB_ALLOWED = 0xFFFFF | QUEST_TRILLIUMEMU_FLAGS_REPEATABLE | QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT | QUEST_TRILLIUMEMU_FLAGS_AUTO_ACCEPT | QUEST_TRILLIUMEMU_FLAGS_DF_QUEST,

    // Trillium flags for internal use only
    QUEST_TRILLIUMEMU_FLAGS_DELIVER              = 0x04000000,   // Internal flag computed only
    QUEST_TRILLIUMEMU_FLAGS_SPEAKTO              = 0x08000000,   // Internal flag computed only
    QUEST_TRILLIUMEMU_FLAGS_KILL_OR_CAST         = 0x10000000,   // Internal flag computed only
    QUEST_TRILLIUMEMU_FLAGS_TIMED                = 0x20000000,   // Internal flag computed only
    QUEST_TRILLIUMEMU_FLAGS_PLAYER_KILL          = 0x40000000,   // Internal flag computed only
};
...
class Quest
{
    ...
    public:
        ...
        bool   IsRepeatable() const { return QuestFlags & QUEST_TRILLIUMEMU_FLAGS_REPEATABLE; }
        ...
        bool   IsDFQuest() const { return QuestFlags & QUEST_TRILLIUMEMU_FLAGS_DF_QUEST; }
