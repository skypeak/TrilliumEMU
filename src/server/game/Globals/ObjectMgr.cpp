...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellPair cell_pair = Arkcore::ComputeCellPair(data->posX, data->posY);
...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = mQuestTemplates.begin(); iter != mQuestTemplates.end(); ++iter)
    {
        ...
        if (qinfo->QuestFlags & ~QUEST_TRILLIUMEMU_FLAGS_DB_ALLOWED)
        {
            sLog->outErrorDb("Quest %u has `SpecialFlags` = %u > max allowed value. Correct `SpecialFlags` to value <= %u",
                qinfo->GetQuestId(), qinfo->QuestFlags  >> 20, QUEST_TRILLIUMEMU_FLAGS_DB_ALLOWED >> 20);
            qinfo->QuestFlags &= QUEST_TRILLIUMEMU_FLAGS_DB_ALLOWED;
        }
        ...
        {
            if (!(qinfo->QuestFlags & QUEST_TRILLIUMEMU_FLAGS_REPEATABLE))
            {
                sLog->outErrorDb("Daily Quest %u not marked as repeatable in `SpecialFlags`, added.", qinfo->GetQuestId());
                qinfo->QuestFlags |= QUEST_TRILLIUMEMU_FLAGS_REPEATABLE;
            }
        }
        ...
        for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_TRILLIUMEMU_FLAGS_DELIVER);
        ...
        for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                if (!qinfo->ReqCreatureOrGOId[j])
                {
                    ...
                    if (found)
                    {
                        if (!qinfo->HasFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT))
                        {
                            sLog->outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and ReqCreatureOrGOId%d = 0, but quest not have flag QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT. Quest flags or ReqCreatureOrGOId%d must be fixed, quest modified to enable objective.", spellInfo->Id, qinfo->QuestId, j+1, j+1);

                            // this will prevent quest completing without objective
                            const_cast<Quest*>(qinfo)->SetFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT);
                        }
                    }
                    else
                    {
        ...
        for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_TRILLIUMEMU_FLAGS_KILL_OR_CAST | QUEST_TRILLIUMEMU_FLAGS_SPEAKTO);
        ...
        if (qinfo->LimitTime)
            qinfo->SetFlag(QUEST_TRILLIUMEMU_FLAGS_TIMED);
        if (qinfo->PlayersSlain)
            qinfo->SetFlag(QUEST_TRILLIUMEMU_FLAGS_PLAYER_KILL);
    }

    // check QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT for spell with SPELL_EFFECT_QUEST_COMPLETE
    for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
    {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            if (!quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT))
            {
                sLog->outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE for quest %u , but quest not have flag QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT. Quest flags must be fixed, quest modified to enable objective.", spellInfo->Id, quest_id);

                // this will prevent quest completing without objective
                const_cast<Quest*>(quest)->SetFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT);
            }
        }
    }
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (!quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT))
                {
                    sLog->outErrorDb("Table `%s` has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, but quest not have flag QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT in quest flags. Script command or quest flags wrong. Quest modified to require objective.",
                        tableName.c_str(), tmp.QuestExplored.QuestID, tmp.id);
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    do
    {
        ...
        if (!quest->HasFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT))
        {
            sLog->outErrorDb("Table `areatrigger_involvedrelation` has record (id: %u) for not quest %u, but quest not have flag QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT. Trigger or quest flags must be fixed, quest modified to require objective.", trigger_ID, quest_ID);
...
bool ObjectMgr::LoadArkcoreStrings(char const* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        ...
        if (min_value == MIN_TRILLIUMEMU_STRING_ID)              // error only in case internal strings
    ...
    if (min_value == MIN_TRILLIUMEMU_STRING_ID)
        sLog->outString(">> Loaded %u Arkcore strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetArkcoreString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        ...
        sLog->outErrorDb("Arkcore string entry %i not found in DB.", entry);
    return "<error>";
}
...
{
    if (entry >= TRILLIUMEMU_TRAINER_START_REF)
        return;
...
bool LoadArkcoreStrings(char const* table, int32 start_value, int32 end_value)
{
    ...
    {
        sLog->outErrorDb("Table '%s' load attempted with range (%d - %d) reserved by Arkcore, strings not loaded.", table, start_value, end_value+1);
        return false;
    }
