    ...
    for (DoorInfoMap::iterator itr = lower; itr != upper; ++itr)
    {
        ...
        if (add)
        {
            ...
            switch (data.boundary)
            {
                ...
                case BOUNDARY_N:
                case BOUNDARY_S:
                    data.bossInfo->boundary[itr->second.boundary] = door->GetPositionX();
                    break;
                case BOUNDARY_E:
                case BOUNDARY_W:
                    data.bossInfo->boundary[itr->second.boundary] = door->GetPositionY();
                    break;
                case BOUNDARY_NW:
                case BOUNDARY_SE:
                    data.bossInfo->boundary[itr->second.boundary] = door->GetPositionX() + door->GetPositionY();
                    break;
                case BOUNDARY_NE:
                case BOUNDARY_SW:
                    data.bossInfo->boundary[itr->second.boundary] = door->GetPositionX() - door->GetPositionY();
            ...
            data.bossInfo->door[itr->second.type].erase(door);
    }
