...
class InstanceSave
{
    ...
    public:
        ...
        void AddPlayer(Player* player) { TRILLIUMEMU_GUARD(ACE_Thread_Mutex, _lock); m_playerList.push_back(player); }
        bool RemovePlayer(Player* player) { TRILLIUMEMU_GUARD(ACE_Thread_Mutex, _lock); m_playerList.remove(player); return UnloadIfEmpty(); }
