...
void CreatureEventAIMgr::LoadCreatureEventAI_Summons()
{
    ...
    do
    {
        ...
        if (!Arkcore::IsValidMapCoord(temp.position_x, temp.position_y, temp.position_z, temp.orientation))
        {
        ...
        for (uint32 j = 0; j < MAX_ACTIONS; j++)
        {
            ...
            switch (action.type)
            {
                ...
                case ACTION_T_QUEST_EVENT:
                    ...
                    {
                        if (!qid->HasFlag(QUEST_TRILLIUMEMU_FLAGS_EXPLORATION_OR_EVENT))
