...
#ifndef TRILLIUMEMU_CELLIMPL_H
#define TRILLIUMEMU_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Trillium::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Trillium::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Trillium::ComputeCellCoord(x + radius, y + radius).normalize();
