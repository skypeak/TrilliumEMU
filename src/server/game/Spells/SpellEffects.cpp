...
{
    // NOT USED BY ANY SPELL OR USELESS OR IMPLEMENTED IN DIFFERENT WAY IN TRILLIUMEMU
}
        ...
        if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
                ...
                && (interruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT) && curSpellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE)
            {
                if (m_originalCaster)
                {
                    ...
                    unitTarget->ProhibitSpellSchool(curSpellInfo->GetSchoolMask(), duration/*GetDuration(m_spellInfo)*/);
                }
