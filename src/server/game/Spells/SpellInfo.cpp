...
void SpellInfo::LoadSpellEffects(SpellEffectEntry const* spellEffect)
{
    if (!spellEffect)
        return;

    uint32 _effIndex = spellEffect->EffectIndex;
    if (_effIndex > MAX_SPELL_EFFECTS)
        return;

    Effects[_effIndex] = SpellEffectInfo(spellEffect, this);
}