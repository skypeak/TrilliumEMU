...
void Unit::RemoveAreaAurasDueToLeaveWorld()
{
    ...
    for (AuraMap::iterator iter = m_ownedAuras.begin(); iter != m_ownedAuras.end();)
    {
        Aura* aura = iter->second;
        ++iter;
        Aura::ApplicationMap const& apmap = aura->GetApplicationMap();
        for (Aura::ApplicationMap::const_iterator itr = apmap.begin(); itr!= apmap.end();)
        {
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    if (creatureAttacker && creatureAttacker->GetCreatureInfo()->type_flags[0] & CREATURE_TYPEFLAGS_UNK26)
        return false;
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    if (GetReactionTo(target) <= REP_NEUTRAL
        && target->GetReactionTo(this) <= REP_NEUTRAL
        && (!ToCreature() || !(ToCreature()->GetCreatureInfo()->type_flags[0] & CREATURE_TYPEFLAGS_UNK26)))
        return false;
        ...
        && !((target->GetByteValue(UNIT_FIELD_BYTES_2, 1) & UNIT_BYTE2_FLAG_PVP)))
    {
        if (Creature const* creatureTarget = target->ToCreature())
            return creatureTarget->GetCreatureInfo()->type_flags[0] & CREATURE_TYPEFLAGS_UNK26 || creatureTarget->GetCreatureInfo()->type_flags[0] & CREATURE_TYPEFLAGS_AID_PLAYERS;
    }
    return true;
}
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (Player* playerVictim = victim->ToPlayer())
    {
        ...
        if ((durabilityLoss && !player && !victim->ToPlayer()->InBattleground()) || (player && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP)))
        {
            ...
            playerVictim->DurabilityLossAll(sWorld->getRate(RATE_DURABILITY_LOSS_ON_DEATH), false);
...
void Unit::BuildMovementPacket(ByteBuffer *data) const
{
    ...
    //Enable it when is fixed. data->writeBit(splineElevation);
...
{
    bool isfriendly = IsFriendlyTo(victim);
