...
void OPvPCapturePointZM_Beacon::ChangeState()
{
    ...
    if (m_OldState == OBJECTIVESTATE_ALLIANCE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetArkcoreStringForDBCLocale(ZMBeaconLoseA[m_TowerType]));
    }
    ...
    else if (m_OldState == OBJECTIVESTATE_HORDE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetArkcoreStringForDBCLocale(ZMBeaconLoseH[m_TowerType]));
    }
    ...
    if (retval >= 0)
    {
        if (player->HasAura(ZM_BATTLE_STANDARD_A) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_H)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetArkcoreStringForDBCLocale(LANG_OPVP_ZM_LOSE_GY_H));
        ...
        else if (player->HasAura(ZM_BATTLE_STANDARD_H) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_A)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetArkcoreStringForDBCLocale(LANG_OPVP_ZM_LOSE_GY_A));
