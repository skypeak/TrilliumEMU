...
void OPvPCapturePointNA::FactionTakeOver(uint32 team)
{
    ...
    if (m_ControllingFaction == ALLIANCE)
        sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE, sObjectMgr->GetArkcoreStringForDBCLocale(LANG_OPVP_NA_LOSE_A));
    else if (m_ControllingFaction == HORDE)
        sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE, sObjectMgr->GetArkcoreStringForDBCLocale(LANG_OPVP_NA_LOSE_H));
