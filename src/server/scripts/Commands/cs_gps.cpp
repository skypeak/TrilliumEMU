...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, const char *args)
    {
        ...
        CellCoord cellCoord = Trillium::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);

        uint32 zone_id, area_id;
        object->GetZoneAndAreaId(zone_id, area_id);

        MapEntry const* mapEntry = sMapStore.LookupEntry(object->GetMapId());
        AreaTableEntry const* zoneEntry = GetAreaEntryByAreaID(zone_id);
        AreaTableEntry const* areaEntry = GetAreaEntryByAreaID(area_id);

        float zone_x = object->GetPositionX();
        float zone_y = object->GetPositionY();

        Map2ZoneCoordinates(zone_x, zone_y, zone_id);

        Map const *map = object->GetMap();
        float ground_z = map->GetHeight(object->GetPositionX(), object->GetPositionY(), MAX_HEIGHT);
        float floor_z = map->GetHeight(object->GetPositionX(), object->GetPositionY(), object->GetPositionZ());

        GridCoord gridCoord = Trillium::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());

        // 63? WHY?
        int gx = 63 - gridCoord.x_coord;
        int gy = 63 - gridCoord.y_coord;

        uint32 have_map = Map::ExistMap(object->GetMapId(), gx, gy) ? 1 : 0;
        uint32 have_vmap = Map::ExistVMap(object->GetMapId(), gx, gy) ? 1 : 0;
        ...
        ZLiquidStatus res = map->getLiquidStatus(object->GetPositionX(), object->GetPositionY(), object->GetPositionZ(), MAP_ALL_LIQUIDS, &liquid_status);
        if (res)
        {
